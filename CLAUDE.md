...
- `settings.create_fission_neutrons = False` (fission as capture, so a fixed-source run in a multiplying model finishes) becomes a bare `NONU` card: fission is treated as capture in every cell, and fission gammas are still produced (manual 5.6.7, p. 319; no card means real fission). OpenMC's flag stops fission neutrons only, so the two differ once photons are transported, which `remediate` says in a note. An eigenvalue run with the flag off is refused, since KCODE needs fission neutrons. `validate_deck` checks the card and the model agree both ways.
