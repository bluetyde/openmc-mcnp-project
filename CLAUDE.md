...
- The geometry check is sampling-based evidence, not a proof: small features can be missed, and it supports only P (4-constant), PX/PY/PZ, SO/S/SX/SY/SZ, CX/CY/CZ and C/X/C/Y/C/Z surfaces in universe 0 without transformations. Decks outside that are reported as "could not run", never silently passed.
